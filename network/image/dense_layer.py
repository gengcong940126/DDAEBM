...
class conv2d(nn.Module):
   def __init__(self,in_planes, out_planes, kernel_size=(3, 3), stride=1, dilation=1, padding=1, bias=True, padding_mode='zeros',
       init_scale=1.,spec_norm=False,use_scale=False):
       super().__init__()
       ...
       if use_scale:
            self.g = torch.nn.Parameter(torch.ones(out_planes))
       variance_scaling_init_(self.conv.weight, scale=init_scale)
       if bias:
