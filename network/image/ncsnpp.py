...
def extract(input, t, shape):
  ...
  return out
@utils.register_model(name='ncsn_d')
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.num_res_blocks = num_res_blocks = config.num_res_blocks_E
  ...
  def forward(self, x, time_cond):
    ...
    #   h = h / used_sigmas
    #
    h = -((x - h).square() / 2).sum(dim = [1,2,3])
    #h = -((x - h)/ 2 /extract(self.coeff.sigmas, time_cond + 1, x.shape)).square().sum(dim=[1, 2, 3])
    # sigma = self.coeff.sigmas[time_cond + 1]
    # sigma_cum = self.coeff.sigmas_cum[time_cond+1]
    # c_t_square = sigma_cum
    # mcmc_step_size_b_square = 1
    # step_square =  mcmc_step_size_b_square * sigma ** 2
    # h=h/step_square
    #h=h.mean([1,2,3])
    #h = -(h.square() / 2).sum(dim=[1, 2, 3])
    return h[:,None]
